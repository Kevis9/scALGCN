...
def input_data(DataDir,Rgraph=True):
    ...
    # DataPath2 = '{}/query_data.csv'.format(DataDir)
    # LabelsPath1 = '{}/ref_label.csv'.format(DataDir)
    # LabelsPath2 = '{}/query_label.csv'.format(DataDir)

    #' read the data
    data1 = pd.read_csv(DataPath1, index_col=0, sep=',')
    print('ref_data ', data1.shape)
    data2 = pd.read_csv(DataPath2, index_col=0, sep=',')
    print('query_data ', data2.shape)
    ...
    types = np.unique(np.concatenate([lab_label1['type'], lab_label2['type']])).tolist()
