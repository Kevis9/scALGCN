...
data1 = t(as.data.frame(read.csv(paste(path, 'mouse_data.csv', sep='/'), row.names=1))) # 变为gene * cell
data2 = t(as.data.frame(read.csv(paste(path, 'human_data.csv', sep='/'), row.names=1)))


print(dim(data1))
print(dim(data2))
label1 = as.data.frame(read.csv(paste(path, 'mouse_label.csv', sep='/')))
rownames(label1) = colnames(data1)
label2 = as.data.frame(read.csv(paste(path, 'human_label.csv', sep='\\')))
